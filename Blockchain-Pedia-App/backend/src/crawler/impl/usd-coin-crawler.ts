...
export class UsdCoinCrawler extends WebCrawler {
    private static readonly COINGECKO_COIN_ID = "usd-coin"

    private static readonly BLOCKCHAIN_NAME: string = "USD Coin (USDC)"
    private static readonly BLOCKCHAIN_URL: string = "https://centre.io/usdc"
    ...
    async crawl(): Promise<BlockchainModel> {
        ...
        let httpResponse: HttpJsonResponse = await Promise.race([this.coinGeckoApi.getCoinCurrentData(UsdCoinCrawler.COINGECKO_COIN_ID), this.coinGeckoApi.timeout(WebCrawler.delayTime)])
        ...
        return new BlockchainModelBuilder()
            .withName(UsdCoinCrawler.BLOCKCHAIN_NAME)
            .withBaseUrl(UsdCoinCrawler.BLOCKCHAIN_URL)
            .withCategory(UsdCoinCrawler.BLOCKCHAIN_CATEGORY)
            .withPowerConsumption(UsdCoinCrawler.BLOCKCHAIN_POWER_CONSUMPTION)
