...
export class OkbCrawler extends WebCrawler {
    private static readonly COINGECKO_COIN_ID = "okb"

    private static readonly BLOCKCHAIN_NAME: string = "OkbCrawler"
    private static readonly BLOCKCHAIN_URL: string = "https://okx.com/"
    private static readonly BLOCKCHAIN_CATEGORY: BlockchainCategory = BlockchainCategory.PUBLIC
    private static readonly BLOCKCHAIN_POWER_CONSUMPTION: number = 0.073 // in MWh
    ...
    async crawl(): Promise<BlockchainModel> {
        ...
        let httpResponse: HttpJsonResponse = await Promise.race([this.coinGeckoApi.getCoinCurrentData(OkbCrawler.COINGECKO_COIN_ID), this.coinGeckoApi.timeout(WebCrawler.delayTime)])
        ...
        return new BlockchainModelBuilder()
            .withName(OkbCrawler.BLOCKCHAIN_NAME)
            .withBaseUrl(OkbCrawler.BLOCKCHAIN_URL)
            .withCategory(OkbCrawler.BLOCKCHAIN_CATEGORY)
            .withPowerConsumption(OkbCrawler.BLOCKCHAIN_POWER_CONSUMPTION)
