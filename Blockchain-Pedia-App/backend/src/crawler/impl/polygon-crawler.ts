...
export class PolygonCrawler extends WebCrawler {
    private static readonly COINGECKO_COIN_ID = "matic-network"

    private static readonly BLOCKCHAIN_NAME: string = "Polygon (MATIC)"
    private static readonly BLOCKCHAIN_URL: string = "https://polygon.technology/"
    private static readonly BLOCKCHAIN_CATEGORY: BlockchainCategory = BlockchainCategory.PUBLIC
    private static readonly BLOCKCHAIN_POWER_CONSUMPTION: number = 0.014 // in MWh
    ...
    async crawl(): Promise<BlockchainModel> {
        ...
        let httpResponse: HttpJsonResponse = await Promise.race([this.coinGeckoApi.getCoinCurrentData(PolygonCrawler.COINGECKO_COIN_ID), this.coinGeckoApi.timeout(WebCrawler.delayTime)])
        ...
        return new BlockchainModelBuilder()
            .withName(PolygonCrawler.BLOCKCHAIN_NAME)
            .withBaseUrl(PolygonCrawler.BLOCKCHAIN_URL)
            .withCategory(PolygonCrawler.BLOCKCHAIN_CATEGORY)
            .withPowerConsumption(PolygonCrawler.BLOCKCHAIN_POWER_CONSUMPTION)
