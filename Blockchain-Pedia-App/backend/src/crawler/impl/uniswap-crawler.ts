...
export class UniswapCrawler extends WebCrawler {
    private static readonly COINGECKO_COIN_ID = "uniswap"

    private static readonly BLOCKCHAIN_NAME: string = "Uniswap (UNI)"
    private static readonly BLOCKCHAIN_URL: string = "https://uniswap.org/"
    private static readonly BLOCKCHAIN_CATEGORY: BlockchainCategory = BlockchainCategory.PUBLIC
    private static readonly BLOCKCHAIN_POWER_CONSUMPTION: number = 0.128 // in MWh
    ...
    async crawl(): Promise<BlockchainModel> {
        ...
        let httpResponse: HttpJsonResponse = await Promise.race([this.coinGeckoApi.getCoinCurrentData(UniswapCrawler.COINGECKO_COIN_ID), this.coinGeckoApi.timeout(WebCrawler.delayTime)])
        ...
        return new BlockchainModelBuilder()
            .withName(UniswapCrawler.BLOCKCHAIN_NAME)
            .withBaseUrl(UniswapCrawler.BLOCKCHAIN_URL)
            .withCategory(UniswapCrawler.BLOCKCHAIN_CATEGORY)
            .withPowerConsumption(UniswapCrawler.BLOCKCHAIN_POWER_CONSUMPTION)
