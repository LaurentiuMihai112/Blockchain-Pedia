...
export class DaiCrawler extends WebCrawler {
    private static readonly COINGECKO_COIN_ID = "dai"

    private static readonly BLOCKCHAIN_NAME: string = "Dai (DAI)"
    private static readonly BLOCKCHAIN_URL: string = "https://makerdao.com/"
    ...
    async crawl(): Promise<BlockchainModel> {
        ...
        let httpResponse: HttpJsonResponse = await Promise.race([this.coinGeckoApi.getCoinCurrentData(DaiCrawler.COINGECKO_COIN_ID), this.coinGeckoApi.timeout(WebCrawler.delayTime)])
        ...
        return new BlockchainModelBuilder()
            .withName(DaiCrawler.BLOCKCHAIN_NAME)
            .withBaseUrl(DaiCrawler.BLOCKCHAIN_URL)
            .withCategory(DaiCrawler.BLOCKCHAIN_CATEGORY)
            .withPowerConsumption(DaiCrawler.BLOCKCHAIN_POWER_CONSUMPTION)
